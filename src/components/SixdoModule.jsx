...
export default function SixdoModule() {
  const [selectedImg, setSelectedImg] = useState(null);
  const images = siteConfig.sixdoModule.images || [];
  ...
  return (
    <section id="sixdo-module" className="relative md:min-h-screen w-full bg-graphite flex flex-col justify-center py-12 md:py-20 px-6 md:px-12 border-b border-charcoal overflow-hidden blueprint-grid-fine">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col mb-12 relative pl-6 border-l-2 border-gold/60">
          ...
          <p className="text-xs font-mono text-gold/70 uppercase tracking-[0.2em] mt-1">
            {siteConfig.sixdoModule.subtitle}
          </p>
        </div>
        ...
        <div className="relative p-6 md:p-8 border border-charcoal/80 bg-charcoal/20 backdrop-blur-md rounded-sm gold-glow w-full overflow-hidden">
          ...
          <div className="flex justify-between items-center border-b border-charcoal/60 pb-3 mb-6 font-mono text-[9px] text-platinum/50">
            <span className="flex items-center gap-2">
              ...
              TACTICAL_GRID // SIXDO_EVENT_MONITOR
            </span>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none stroke-gold/10 fill-none" xmlns="http://www.w3.org/2000/svg">
            {/* Grid connecting lines matching the asymmetric 2+4 layout */}
            <line x1="66.6%" y1="0%" x2="66.6%" y2="65%" strokeDasharray="3,4" className="hidden lg:block" />
            <line x1="25%" y1="65%" x2="25%" y2="100%" strokeDasharray="3,4" className="hidden lg:block" />
            <line x1="50%" y1="65%" x2="50%" y2="100%" strokeDasharray="3,4" className="hidden lg:block" />
            <line x1="75%" y1="65%" x2="75%" y2="100%" strokeDasharray="3,4" className="hidden lg:block" />
            <line x1="0%" y1="65%" x2="100%" y2="65%" strokeDasharray="3,4" className="hidden lg:block" />
            
            {/* Circuit paths to tie images visually */}
            <path d="M 66.6,150 L 66.6,400 L 25,400 L 25,550" stroke="rgba(213, 182, 122, 0.25)" strokeWidth="1.2" className="hidden lg:block" />
            <path d="M 10,150 L 10,400 L 500,400" stroke="rgba(213, 182, 122, 0.15)" strokeWidth="1" className="hidden lg:block" />
            
            {/* Pulsing junction nodes at wire crossings */}
            <circle cx="66.6%" cy="65%" r="3.5" fill="#D5B67A" className="animate-ping hidden lg:block" />
            <circle cx="66.6%" cy="65%" r="2" fill="#D5B67A" className="hidden lg:block" />
            <circle cx="25%" cy="65%" r="3.5" fill="#D5B67A" className="animate-ping hidden lg:block" />
            <circle cx="25%" cy="65%" r="2" fill="#D5B67A" className="hidden lg:block" />
            <circle cx="50%" cy="65%" r="3.5" fill="#D5B67A" className="animate-ping hidden lg:block" />
            <circle cx="50%" cy="65%" r="2" fill="#D5B67A" className="hidden lg:block" />
            <circle cx="75%" cy="65%" r="3.5" fill="#D5B67A" className="animate-ping hidden lg:block" />
            <circle cx="75%" cy="65%" r="2" fill="#D5B67A" className="hidden lg:block" />
          </svg>
          ...
          <div className="flex justify-between items-center border-t border-charcoal/60 pt-3 mt-6 font-mono text-[8px] text-platinum/40">
            <span>SYS_COORD: SD_SYS_LINK // 100% STABLE</span>
        ...
        {selectedImg !== null && (
          ...
          >
            ...
            <div className="relative flex-grow w-full flex items-center justify-center my-4 z-10">
              <AnimatePresence mode="wait">
                <motion.img 
                  ...
                  alt={`${siteConfig.sixdoModule.title} Lightbox ${selectedImg + 1}`}
            ...
            <div className="w-full border-t border-charcoal/80 pt-4 font-mono text-[9px] text-platinum/40 flex justify-between items-center z-10">
              <span>SCAN_NODE: CAM_SD_0{selectedImg + 1} // BUFFER: 100% // RESOLUTION: HIGH_RES</span>
